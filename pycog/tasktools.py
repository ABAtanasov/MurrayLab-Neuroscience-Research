...
def generate_ei(N, pE=0.8):
    ...
    idx = range(N)
    EXC = idx[:Nexc]             # numbers of the excitatory neurons
    INH = idx[Nexc:]             # numbers of the inhibitory neurons

    ei       = np.ones(N, dtype=int) 
    ei[INH] *= -1                #an array of Nexc 1s and then Ninh -1s
...
def performance_2afc(trials, z):
    ends    = [len(trial['t'])-1 for trial in trials]   #final times for each trial
    choices = [np.argmax(z[ends[i],i]) for i, end in enumerate(ends)]
    correct = [choice == trial['info']['choice']
               for choice, trial in zip(choices, trials) if trial['info']]
                #correct counts how many times the choice is right
                #choices is the arg max of the z at each termination time
...
def performance_2afc_min_condition(trials, z):
    ...
    for choice, trial in zip(choices, trials):
        if not trial['info']:       #only count trials that have info
            continue
